...
async def set_gosl_handler(message: types.Message, command: CommandObject):
    ...
    if not args:
        await message.reply(
            'Формат: /set_goal 100000 "Скопить на отпуск"\nСумма и описание обязательны.'
        )
        return
    ...
    if len(elems) < 2:
        await message.reply(
            "Укажите сумму и описание цели. Пример: /set_goal 100000 Скопить на отпуск"
        )
        return
...
async def get_goals_handler(message: types.Message):
    async with sessionLocal() as db:
        ...
        for goal in goals:
            ...
            status = (
                "✅ Достигнута!"
                if balance >= goal.summa
                else f"📈 Прогресс: {progress:.1f}%"
            )
