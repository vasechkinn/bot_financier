...
async def add_income(db: AsyncSession, tg_id: int, data: IncomeOp) -> Transaction:
...
async def add_expense(db: AsyncSession, tg_id: int, data: ExpenseOp) -> Transaction:
...
async def get_transactions(
    db: AsyncSession, tg_id: int, period: str, category: str | None = None
):
    user_id_db = await get_user_id(db, tg_id)
    ...
    time_select = (
        select(Transaction)
        .where(Transaction.user_id == user_id_db, Transaction.date >= date_start)
    ...
    all_expenses = sum(
        transaction.summa
        for transaction in transactions
        if transaction.operation_type == OperationType.EXPENSE
    )

    return transactions, all_incomes, all_expenses


async def get_balance(db: AsyncSession, tg_id: int):
    user_id_db = await get_user_id(db, tg_id)
    ...
    income_sum = sum(
        t.summa for t in transactions if t.operation_type == OperationType.INCOME
    )
    expense_sum = sum(
        t.summa for t in transactions if t.operation_type == OperationType.EXPENSE
    )

    return income_sum - expense_sum


async def set_goal(db: AsyncSession, tg_id: int, description: str, summa: float):
    user_id_db = await get_user_id(db, tg_id)
...
async def get_active_goal(
    ...
    select_transactions = select(Goal).where(
        Goal.user_id == user_id_db, Goal.completion_status == False
    )
...
async def set_log_pass(db: AsyncSession, tg_id: int, login: str, password: str):
